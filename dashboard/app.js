...
function computedVehicleStatus(g) {
  if (g.status_operacional !== "disponivel") return g.status_operacional;
...
function renderSummary() {
  const dispatch = ocorrencias.filter(isDispatchOccurrence).length,
    progress = ocorrencias.filter(isProgressOccurrence).length,
    available = guarnicoes.filter(
      (g) => computedVehicleStatus(g) === "disponivel",
    ).length,
    today = new Date().toISOString().slice(0, 10),
    closedToday = ocorrencias.filter(
      (o) =>
